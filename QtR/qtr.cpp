...
void RQtTest::createDockWindows()
{
    ...
    dockin = new QDockWidget(tr("script"), this);
    dockout = new QDockWidget(tr("console"), this);
    ...
    input->setFont(f);
    input->setText(tr("# input R code and press CTRL + R to run") );
...
void RQtTest::keyPressEvent(QKeyEvent *ev)
{
    ...
    if ((ev->key() == Qt::Key_R)  && QApplication::keyboardModifiers() && Qt::ControlModifier)
        runCmd();
}
